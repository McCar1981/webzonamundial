...
import { monedasPorCalificacion, puntosPorCalificacion, calificacionPorPuntaje } from "@/lib/draft/simulacion";
...
export async function POST(req: Request) {
  ...
  // ANTI-FALSIFICACIÓN: el puntaje se acota a 0-100 y la calificación se
  // RECALCULA aquí a partir del puntaje (escala vigente). Antes ambos entraban
  // tal cual llegaban del cliente: un POST manual con puntaje 999 o
  // calificacion "Leyenda" envenenaba el ranking (las monedas ya estaban
  // acotadas, pero la tabla no). TODO lo que sigue usa los valores seguros.
  const stat = (v: unknown) => Math.min(100, Math.max(0, Math.round(Number(v) || 0)));
  const safePuntaje = stat(puntaje);
  const safeCalificacion = calificacionPorPuntaje(safePuntaje);
  ...
  const baseRow = {
    ...
    coherencia: stat(coherencia),
    bonus_estilo: stat(bonusEstilo),
