...
const LOGROS = [
  { icon: "👑", nombre: "Leyenda Viva", desc: "Obtené calificación Leyenda" },
  { icon: "🌍", nombre: "Arquitecto", desc: "3+ jugadores de la misma selección" },
  { icon: "🧠", nombre: "De Memoria", desc: "Oro+ en modo Almanaque" },
  { icon: "⏰", nombre: "Contra el Tiempo", desc: "Completá un draft en Contrarreloj" },
  { icon: "🛡️", nombre: "La Muralla", desc: "Todos los defensores con 85+" },
  { icon: "📜", nombre: "Historiador", desc: "Jugadores de 5+ selecciones distintas" },
];
...
export default function DraftMundialPage() {
  return (
    <div className="min-h-screen" style={{ background: NAVY }}>
      ...
      {/* Logros */}
      <div className="px-4 py-12 max-w-xl mx-auto">
        <h2 className="text-xl font-bold text-center mb-2" style={{ color: TXT }}>
          🏆 Logros por desbloquear
        </h2>
        <p className="text-sm text-center mb-6" style={{ color: TXT_MUT }}>
          Cada draft es una oportunidad de conseguir un nuevo badge
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {LOGROS.map((l) => (
            <div key={l.nombre} className="p-3 rounded-xl border text-center" style={{ background: CARD, borderColor: "rgba(255,255,255,0.08)" }}>
              <div className="text-2xl mb-1">{l.icon}</div>
              <div className="text-xs font-bold" style={{ color: TXT }}>{l.nombre}</div>
              <div className="text-[10px]" style={{ color: TXT_MUT }}>{l.desc}</div>
            </div>
          ))}
        </div>
      </div>
