    ...
    <div>
      <div style={{ background: `${GOLD}14`, border: `1px solid ${GOLD}44`, borderRadius: 12, padding: "12px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: GOLD2, flex: 1, minWidth: 200 }}>
          Crea tu liga privada, ponle nombre e invita a tus amigos con un código. Inicia sesión para competir de verdad.
        </span>
      ...
      <div style={{ background: BG2, border: `1px solid ${GOLD}33`, borderRadius: 14, padding: 16, marginBottom: 14, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 16 }}>
        <div>
          <div style={{ fontSize: 12, fontWeight: 800, color: GOLD2, marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}><IconPlus />Crear liga privada</div>
...
function RealLeagues({ team }: { team: FantasyTeamState }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (active === "global") {
        ...
        if (!cancel) { setStandings(r.standings); setLeagueOwner(r.is_owner); }
      }
  ...
  const leave = async (id: string, owner: boolean) => {
    ...
    setMsg(action === "deleted" ? "Liga borrada." : "Has salido de la liga.");
  };
  const rename = async (id: string) => {
    const name = renameVal.trim();
    if (!name) { setMsg("Escribe un nombre."); return; }
    const ok = await renameServerLeague(id, name);
    if (ok) { await reloadLeagues(); setRenaming(false); setMsg("Liga renombrada."); }
    else setMsg("No se pudo renombrar.");
  };
  const kick = async (id: string, memberId: string, name: string) => {
    ...
    if (ok) { const r = await fetchLeagueStandings(id, leaguePeriod === "weekly" ? currentGw : undefined); setStandings(r.standings); setMsg(`${name} expulsado.`); }
    else setMsg("No se pudo expulsar.");
  };
    ...
    <div>
      ...
      {activeLeague && (
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 12 }}>
          ...
          <button onClick={() => { navigator.clipboard?.writeText(activeLeague.code); setMsg(`Código ${activeLeague.code} copiado.`); }} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.15)", background: BG2, color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer" }}><IconCopy />Copiar</button>
      ...
      {loading ? (
        ...
        <LeagueTable rows={standings ?? []} canKick={leagueOwner} onKick={activeLeague ? (uid, name) => kick(activeLeague.id, uid, name) : undefined} />
      )}
    </div>
  );
}
...
function LeagueTable({ rows, canKick, onKick }: { rows: FantasyLeagueStanding[]; canKick?: boolean; onKick?: (uid: string, name: string) => void }) {
  if (rows.length === 0) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      {rows.map((r) => (
        <div key={r.user_id} style={{ display: "flex", alignItems: "center", gap: 12, background: BG2, border: "1px solid rgba(255,255,255,0.05)", borderRadius: 10, padding: "10px 14px" }}>
          <span style={{ width: 26, textAlign: "center", fontSize: 14, fontWeight: 900, color: r.position === 1 ? GOLD2 : r.position <= 3 ? "#fff" : DIM }}>{r.position}</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", display: "flex", alignItems: "center", gap: 5 }}>{r.team_name}{r.is_owner && <IconCrown size={13} color={GOLD2} />}</div>
            <div style={{ fontSize: 11, color: MID }}>{r.display_name}</div>
          </div>
          <span style={{ fontSize: 15, fontWeight: 800, fontVariantNumeric: "tabular-nums" }}>{r.points}</span>
